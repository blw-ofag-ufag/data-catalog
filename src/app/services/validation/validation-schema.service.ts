...
export class ValidationSchemaService {
	...
	// Property keys declared by the schema currently backing `base`. Used to suppress
	// strict-profile (i14y/ods) requirements for fields the active product type has no
	// input for; see isFieldOnActiveType.
	private baseSchemaKeys: Set<string> = new Set();
	...
	getFilteredSchemaValidationErrors(schemaType: ValidationSchemaType, formValue: unknown): string[] {
		...
		Object.entries(schema.fields).forEach(([fieldKey, validation]) => {
			...
			// The i14y / ods profiles are written against the *dataset* schema. When editing a
			// dataService or datasetSeries they would demand fields that type does not declare
			// (dct:issued, bv:classification, bv:personalData), producing "erforderlich"
			// messages for inputs that do not exist in the form and can never be satisfied
			// (#221, top priority). Only report fields the active type actually has.
			if (!this.isFieldOnActiveType(fieldKey)) {
				return;
			}
	...
	/**
	 * Whether a field exists on the product type currently backing the `base` slot.
	 *
	 * Defaults to true while no schema is loaded, so validation is never silently skipped.
	 */
	private isFieldOnActiveType(fieldKey: string): boolean {
		if (this.baseSchemaKeys.size === 0) return true;
		// Nested keys are addressed as `parent.child` (e.g. dcat:contactPoint.schema:email).
		return this.baseSchemaKeys.has(fieldKey.split('.')[0]);
	}
		...
		this.schemaFetcher.fetchAllSchemas(configs).subscribe({
			next: fetchedSchemas => {
				fetchedSchemas.forEach((config, id) => {
					...
					// Remember which properties the base (product) schema declares, so the strict
					// profiles can't demand fields the active type has no input for (#221).
					if (id === 'base') {
						this.baseSchemaKeys = new Set(Object.keys(config.schema?.properties || {}));
					}
	...
	loadBaseForType(type: DataProductType = DEFAULT_DATA_PRODUCT_TYPE): void {
		...
		this.schemaFetcher.fetchSchema(config).subscribe(schema => {
			...
			this.baseType = type;
			this.baseSchemaKeys = new Set(Object.keys(schema?.properties || {}));
			this.schemas.set('base', {
				id: 'base',
